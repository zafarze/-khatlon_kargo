...
from config import (
    ...
    AWAITING_FULL_NAME, AWAITING_PHONE, AWAITING_ADDRESS, AWAITING_LANG_CHOICE, AWAITING_REGION_CHOICE,
    AWAITING_TRACK_CODE,
...
CONTACT_INSTAGRAM = "khatlonkargo"
CONTACT_PHONES = {
    "Бохтар": "+992936005303",
    "Кабодиён": "+992935398448"
}
...
REGEX_CHANGE_REGION = r"^🌍\s*(Регион|Минтақа|Region).+$"
...
def normalize_region(region_raw: str) -> str:
    """Нормализует сырое значение региона из БД/коллбэка в русский ключ (Бохтар/Кабодиён)."""
    mapping = {
        "Bokhtar": "Бохтар",
        "Qubodiyon": "Кабодиён",
        "qubodiyon": "Кабодиён",
    ...
    return mapping.get(region_raw, "Бохтар")
...
async def get_main_keyboard(user_id: int, lang: str, is_admin: bool = False) -> ReplyKeyboardMarkup:
    ...
    buttons = copy.deepcopy(get_text('main_buttons', lang))
    
    if len(buttons) > 3 and len(buttons[3]) > 1:
        # Formulate dynamic region button text based on language
        en_region = "bokhtar" if region == "Бохтар" else "qubodiyon"
        if lang == 'ru':
            region_str = f"🌍 Регион: {get_text('region_' + en_region, lang)}"
        elif lang == 'tg':
            region_str = f"🌍 Минтақа: {get_text('region_' + en_region, lang)}"
        else:
            region_str = f"🌍 Region: {get_text('region_' + en_region, lang)}"
            
        buttons[3][1] = region_str
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if db_user:
        ...
        await update.message.reply_text(
            ...
            reply_markup=await get_main_keyboard(user_id, lang, is_admin(user_id))
        )
        return MAIN_MENU
...
async def select_language(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    return await start_region_selection(update, context)
...
async def start_region_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    region_keyboard = InlineKeyboardMarkup([
        [
            InlineKeyboardButton(get_text('region_bokhtar', lang), callback_data="region_Bokhtar"),
            InlineKeyboardButton(get_text('region_qubodiyon', lang), callback_data="region_Qubodiyon")
        ]
    ])
    ...
    return AWAITING_REGION_CHOICE

async def select_region(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    
    region_raw = query.data.split('_')[1] 
    region_mapping = {"Bokhtar": "Бохтар", "Qubodiyon": "Кабодиён"}
    db_region = region_mapping.get(region_raw, "Бохтар")
    
    context.user_data['region'] = db_region
    lang = context.user_data.get('lang', 'ru')
    
    logger.info(f"User {update.effective_user.id} selected region: {db_region}")

    try:
        await query.edit_message_text(get_text('region_selected', lang).format(region=get_text('region_' + region_raw.lower(), lang)))
    except Exception as e:
        logger.warning(f"Не удалось отредактировать сообщение о выборе региона: {e}")
...
async def register_address(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        success = await register_user(
            ...
            username=username,
            language_code=lang,
            region=context.user_data.get('region', 'Бохтар')
        )
...
async def show_contacts(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    logger.info(f"show_contacts called for User ID {user_id}. Region fetched from DB: {region}. CONTACT_PHONES keys: {CONTACT_PHONES.keys()}")
    
    from config import PHOTO_CONTACT_PATHS
    photo_path = PHOTO_CONTACT_PATHS.get(region, PHOTO_CONTACT_PATHS['Бохтар'])
    
    phone_number = CONTACT_PHONES.get(region, CONTACT_PHONES['Бохтар'])
    logger.info(f"Resolved phone number: {phone_number}")
    contacts_text = get_text('contacts', lang).format(phone=phone_number)
    
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton(f"📱 Наш Instagram", url=f"https://instagram.com/{CONTACT_INSTAGRAM}")]
    ])
...
async def show_prices(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    from config import PHOTO_PRICE_PATHS
    photo_path = PHOTO_PRICE_PATHS.get(region, PHOTO_PRICE_PATHS['Бохтар'])
...
async def show_address_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Показывает адрес. Для Таджикистана - мгновенная карта + кнопка видео."""
    query = update.callback_query
    await query.answer()
    ...
    if address_type == "china":
        ...
        from config import PHOTO_ADDRESS_TAJIK_PATHS
        photo_path = PHOTO_ADDRESS_TAJIK_PATHS.get(region, PHOTO_ADDRESS_TAJIK_PATHS['Бохтар'])
        ...
        # 3. Отправляем фото адреса с кнопкой (без видео, чтобы не тормозило)
        if os.path.exists(photo_path):
             await send_photo_safe(context, query.message.chat_id, photo_path, caption, reply_markup=keyboard)
        else:
...
async def change_language_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await query.message.reply_text(
            ...
            reply_markup=await get_main_keyboard(user_id, context.user_data.get('lang', 'ru'), is_admin(user_id))
        )
        
    return MAIN_MENU

async def change_region_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    lang = context.user_data.get('lang', 'ru')
    region_keyboard = InlineKeyboardMarkup([
            [
                InlineKeyboardButton(get_text('region_bokhtar', lang), callback_data="change_region_Bokhtar"),
                InlineKeyboardButton(get_text('region_qubodiyon', lang), callback_data="change_region_Qubodiyon")
            ]
        ])
...
async def change_region_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    
    region_raw = query.data.split('_')[2]
    user_id = update.effective_user.id
    ...
    try:
        from db_utils import update_user_region
        success = await update_user_region(user_id, db_region) 
        
        if not success:
            raise Exception("update_user_region returned False")
        ...
        logger.error(f"Ошибка смены региона для {user_id}: {e}")
        lang = context.user_data.get('lang', 'ru')
        await query.message.reply_text(
            get_text('error', lang),
            reply_markup=await get_main_keyboard(user_id, lang, is_admin(user_id))
...
def get_main_conv_handler() -> ConversationHandler:
    ...
    states = {
        ...
        MAIN_MENU: [
            ...
            MessageHandler(lang_filter & filters.Regex(REGEX_CHANGE_REGION), change_region_start),
            
            CallbackQueryHandler(show_address_callback, pattern='^address_'),
            CallbackQueryHandler(change_language_callback, pattern='^set_lang_'),
            CallbackQueryHandler(change_region_callback, pattern='^change_region_'),
            CallbackQueryHandler(link_order_callback, pattern='^link_'),
